...
function ThemContButton() {
    const [theme,setThem] = useState(localStorage.getItem('theme') ? localStorage.getItem('theme') : 'light');
    useEffect(() => {
       let them = localStorage.setItem('theme',theme);
       let localThem = localStorage.getItem('theme');
        ...
        <div>
            <label className="swap swap-rotate mt-1">
                ...
                <input
                    ...
                    onChange={() => setThem(theme === 'light' ? 'dark' : 'light')}
                />

                {/* Sun icon for light mode */}
                <svg
                    className="swap-off h-7 w-7 fill-current mr-10"
